...
/**
 * Abstract MessageSource implementation based on Spring's
 * {@link org.springframework.context.support.AbstractMessageSource}. We can't
 * use the original class because we need to pass the default message to the
 * {@link #resolveCode(String, Locale, String)} method.
 */
public abstract class AbstractMessageSource implements HierarchicalMessageSource {

	private Log log = LogFactory.getLog(AbstractMessageSource.class);
	 ...
	 */
	private final Map<String, MessageFormat> cachedMessageFormats = Generics.newHashMap();
	...
	public final String getMessage(MessageSourceResolvable resolvable, Locale locale)
		...
		for (int i = 0; i < codes.length; i++) {
			String msg = getMessageInternal(codes[i], resolvable.getArguments(), locale, resolvable.getDefaultMessage());
			if (msg != null) {
				return msg;
			}
		}
	...
	protected String getMessageInternal(String code, Object[] args, 
		...
		if (ObjectUtils.isEmpty(args)) {
			...
			String message = resolveCodeWithoutArguments(code, locale, defaultMessage);
			if (message != null) {
				return message;
			}
		}
			...
			MessageFormat messageFormat = resolveCode(code, locale, defaultMessage);
	...
	protected MessageFormat createMessageFormat(String msg, Locale locale) {
		if (log.isDebugEnabled()) {
			log.debug("Creating MessageFormat for pattern [" + msg + "] and locale '" + locale + "'");
		}
	...
	protected Object[] resolveArguments(Object[] args, Locale locale) {
		...
		List<Object> resolvedArgs = Generics.newArrayList(args.length);
