...
<#macro form command="command" attributes...>
	...
	<form${common.joinAttributes(attributes)}>
		<#nested />
	</form>
	${formMacroHelper.end()!}
</#macro>
...
<#macro listErrors field="" tag="" attributes...>
	...
	<#if errors?has_content>
		<#if tag?has_content><${tag + common.joinAttributes(attributes)}></#if>
...
<#macro label for field="" errorClass="error" code="" attributes...>
	<#compress>
		...
		<label for="${for}"${common.joinAttributes(attributes)}>${text}</label>
	</#compress>
</#macro>
...
<#macro input type field id=field errorClass="error" attributes...>
	<#compress>
		...
		<#local attributes = addErrorClass(attributes, field, errorClass) />
	    <input id="${id}" type="${type}" name="${field}" value="${getDisplayValue(field)}"${common.joinAttributes(attributes)} />
	</#compress>
</#macro>
...
<#macro textarea field id=field errorClass="error" attributes...>
	<#local attributes = addErrorClass(attributes, field, errorClass) />
    <textarea id="${id}" name="${field}"${common.joinAttributes(attributes)}>${getDisplayValue(field)}</textarea>
</#macro>
...
<#macro select field options id=field errorClass="error" valueProperty="" 
	...
	<#compress>
		<#local attributes = addErrorClass(attributes, field, errorClass) />
	    <select id="${id}" name="${field}"${common.joinAttributes(attributes)}>
...
<#macro checkbox field id=field errorClass="error" value="on" attributes...>
	<#compress>
		<#local attributes = addErrorClass(attributes, field, errorClass) />
		<input type="checkbox" name="${field}" id="${id}" value="${value}"<@check formMacroHelper.getValue(field)! == value />${common.joinAttributes(attributes)} />
...
<#macro option value label=value selected=false attributes...>
	<#compress>
		<option value="${value}"<#if selected> selected="selected"</#if>${common.joinAttributes(attributes)}>${label}</option>
	</#compress>
</#macro>
